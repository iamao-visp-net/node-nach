...
File.prototype.generateBatches = function(done1) {
  ...
  async.each(this._batches, function(batch, done2) {
    ...
    async.each(batch._entries, function(entry, done3) {
      entry.fields.traceNumber.value = (entry.fields.traceNumber.value) ? entry.fields.traceNumber.value : batch.control.originatingDFI.value.slice(0, 8) + utils.pad(entryCount, 7, false, '0');
